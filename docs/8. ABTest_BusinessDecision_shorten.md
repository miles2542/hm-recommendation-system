...
ALS achieved statistical significance but failed economically. Despite a 9.36% Hit Rate uplift ($Z=3.73$) and passing the Welch's T-Test ($p=0.0234$), the Absolute IRR Shift (+0.000079) yielded a microscopic **Glass's Delta ($\Delta = 0.0110$)**. ALS optimized for structural co-occurrence (Popularity Bias) rather than margin. It increased transaction frequency but cannibalized margin density by recommending cheap basics, proving latent space accuracy does not guarantee basket scale.

**II. Simpson's Paradox and Segment Harm**

The ALS aggregate Hit Rate (+9.36%) is a mathematical artifact of **Simpson's Paradox**. ALS increased conversions for frequent buyers (Mature Champions: +16.76%) but actively harmed dormant segments (**Young At-Risk: -33.55%**, **Mature Newcomers: -19.11%** vs. Control). Collaborative Filtering fails on sparse-history users, feeding them generic popularity lists that underperform the Control's demographically-aware Association Rules.

**III. Treatment B (LightGBM) Dominance: Universal Optimization**

Because trivial repurchases were barred via **Anti-Join**, uplifts represent 100% **Novel Discovery**, proving advanced feature matrices stimulate genuine dormant demand.
...
To avoid the **ROI Trap** (deploying a statistically significant model whose compute costs exceed marginal revenue), the empirical Absolute IRR Shift is projected into an annualized economic forecast for the 1,338,785 segmented users:
...
LightGBM's yield (+22,897 NRU) dwarfs ALS (+1,379 NRU), providing a massive economic margin of safety. Deployment is justified provided monthly cloud inference infrastructure costs remain below $\approx 1,908$ NRU/month (Annual Yield / 12). This threshold must be validated against live billing data prior to enterprise scaling.
...
* **Transition to Supervised Learning:** Deprecate ALS as a primary ranking engine. The catalog's 90-day shelf life mandates Learning-to-Rank architectures capable of utilizing dynamic pricing and demographic features.

* **Enforce "Novel Discovery" Metrics:** Marketing KPIs must reflect the **Anti-Join Architecture**. Replace Gross Hit Rate and AOV with **Net-New Hit Rate** and **Incremental Recommendation Revenue (IRR)** to measure true demand generation, not artificially inflated repurchases.

* **Deploy Dual-Engine Prescriptive Logic:**
  *   **Primary Navigation (LightGBM):** Initiate a **20% Phased Rollout** for Homepage traffic. Monitor real-time guardrails (page latency, checkout drop rate) for 14 days (two weekly cycles) before 100% rollout.
  *   **Shopping Cart (MBA Baseline):** Retain MBA Association Rules at checkout. LightGBM requires heavy batch feature engineering; MBA executes via instant $O(1)$ dictionary lookups, making it computationally optimal for real-time impulse-buy generation.