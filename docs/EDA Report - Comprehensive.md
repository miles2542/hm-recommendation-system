...
- **Null Values**: 
  ...
  - `age` has a 1.16% missing rate, requiring simple median imputation or age-bucket prediction for downstream modeling.
...
- **Technical Note**: Identified using Polars' high-performance duplicate detection: `df.filter(pl.struct(df.columns).is_duplicated())`. These rows must be grouped and aggregated into a `quantity` feature prior to model training to prevent target leakage or skewed engagement metrics.
...
- **Volume Outliers**: The top customer in the dataset purchased **18,950 items** over the two-year period (averaging ~25 items/day). The top 10 customers all exceed 8,000 items.
...
- **RecSys Design**: Standard Matrix Factorization will fail here. Hybrid models leveraging dense item metadata and sequential transformers (e.g., SASRec) are strictly required.
...
3.  **Null Imputation**: 
    ...
    - `customers.age`: Impute with the median (`49`) or build a simple predictive regressor using purchase category affinity (e.g., users who buy `Divided` are highly likely to be in their 20s).
4.  **RecSys Architecture**: Do not attempt standard Collaborative Filtering (CF). The 99.98% sparsity and 90-day SKU shelf life demand a **Content-Based Hybrid** or **Sequential Target** model (like SASRec) that embeds article metadata (`index_name`, `garment_group_name`, `detail_desc` N-grams) alongside historical sequences.
