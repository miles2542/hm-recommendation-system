...
*   **Impact:** Pre-filtering substantially reduces matrix dimensionality prior to instantiation, minimizing memory overhead while successfully approximating the FP-Growth support boundary for high-frequency items. The system securely executed on localized hardware with zero memory spikes, avoiding the arbitrary and unscientific truncation of data (e.g., blindly capping at "Top 5000 items").

### 3.3. Transaction ID (TID) Integrity
*   **Action:** Constructed shopping baskets by grouping `[customer_id, t_dat]` and enforcing a strict `.unique()` array aggregation. Unlike naive implementations, single-item baskets are strictly retained.
*   **Purpose:** The `.unique()` constraint resolves a structural artifact where upstream deduplication collapsed identical SKUs into a `quantity` feature (a standard list aggregation would have generated false "multi-item" baskets like `[Item_A, Item_A]`). 
*   **Impact:** Discarding single-item baskets destroys the mathematical denominator $P(A)$ for Apriori probabilities, leading to artificially inflated Confidence scores. By retaining them, we preserve the statistical purity of the Association Rules. Because Polars pre-pruned the column space to only highly frequent items, the engine securely accommodates the full sparse matrix without computational bloat.
...
*   **VIP Segments ("Mature Champions", "Young Whales"):** The ranking mechanism shifts to prioritize the **Expected Cross-Sell Revenue (ECR)**. 
    $$ECR = Lift \times Confidence \times Normalized\_Consequent\_Price$$
    The Consequent_Median_Price is Min-Max normalized to [0,1] prior to calculation. This ensures it scales harmoniously with Lift and Confidence, preventing raw price magnitude from solely dominating the ranking algorithm.*
...
Empirical analysis of H&M's ecosystem indicates that *repurchase* (e.g., rebuying basics/underwear) is a dominant behavioral signal. Consequently, the algorithm is explicitly designed to tolerate historical repurchases, filtering out only the items currently present in the active cart. This is a deliberate design choice representing an academic trade-off: it maximizes **Hit Rate** (by capturing recurring staple purchases like basics), but intentionally sacrifices **Novelty/Discovery** metrics. Finally, a dynamic padding mechanism ensures the engine consistently outputs a robust vector of exactly $K=12$ items by appending demographic-trending artifacts when rule-based consequents are exhausted.

## 6. Offline Evaluation Results

### 6.1. Evaluation Methodology
To accurately evaluate the associative engine, each user's Last Known Basket from the training period was extracted and passed into the engine to predict their hold-out test period purchases. By querying the chronologically partitioned dataset for the user's absolute maximum interaction date (`last_date`) and extracting only those specific items, we simulate a true Recommender System (RecSys) environment—predicting a user's next sequence of actions based strictly on their most recent contextual state. 

To process this at scale across the hold-out test set, the prediction pipeline was heavily optimized. By constructing a pre-cached Inverted Index (`defaultdict(set)`) and converting antecedent combinations to native Python `frozenset` objects, the engine bypassed slow Pandas row-wise operations, achieving $O(1)$ rule lookups. This allowed the generation of exactly $K=12$ recommendations for over 216,000 users in under 120 seconds.

### 6.2. Final Benchmark Metrics & Detailed Analysis
The model was evaluated against the actual ground truth purchases made during the test window. Below is a detailed breakdown of the performance metrics:

*   **Total Users Evaluated: 216,479**
    *   This represents the exact number of unique customers who made at least one purchase during the hold-out test period. The evaluation strictly filtered for these active users to establish our ground truth. Predicting against a sample size of over 216,000 active shoppers ensures our metrics are statistically highly significant and immune to small-sample variance.
*   **Execution Time: 119.2 seconds**
    *   It took the engine just under two minutes to dynamically construct and rank Top-12 recommendation vectors for all 216,479 users (averaging ~1,800 users processed per second). 
    *   This directly validates the Task 3 code optimization. By replacing 1.5 billion slow Pandas row-wise operations with an $O(1)$ pre-cached Inverted Index and Python `frozenset` lookups, we reduced execution time from over 7 minutes down to 119.2 seconds, proving the engine's scalability for production environments.
*   **MAP@12 (Mean Average Precision): 0.004517**
    *   MAP evaluates not just *if* we recommended the right item, but *where* it was ranked. A correct prediction at rank #1 contributes much more to this score than a correct prediction at rank #12. 
    *   In highly sparse fast-fashion datasets (where users choose from hundreds of thousands of active SKUs), predicting the exact article ID is incredibly difficult. A MAP of ~0.0045 serves as a mathematically rigid, uninflated baseline for positional ranking accuracy.
*   **Hit Rate@12: 4.44% (9,606 / 216,479)**
    *   Out of the 216,479 users evaluated, exactly 9,606 users actually purchased *at least one* item that our engine placed in their Top 12 recommendations.
    *   A 4.44% deterministic Hit Rate is a very strong baseline for a non-personalized, rule-based system. It validates our deliberate design choices—specifically the **Repurchase Tolerance** (which successfully captured users rebuying staple basics) and the **Demographically-Aware Fallback** (which provided culturally relevant trending items when a user's shopping cart didn't trigger any N-to-1 association rules).

### 6.3. Conclusion
These metrics establish a strong, deterministic rule-based baseline that successfully bridges predictive cross-selling with prescriptive, demographically-aware fallback logic. Moving forward, this framework serves as the foundational performance benchmark that the upcoming personalized Collaborative Filtering model (Person D) must beat.

## 7. Data Dictionary: Association Rules Output
The fully enriched, descending-sorted rule dataframe is exported as `Association_Rules.parquet`. This highly compressed artifact serves as the definitive data source for downstream Business Intelligence Dashboards.
...
| `consequent_median_price` | Float64 | The raw median price of the recommended item. |
| `price_norm` | Float64 | The Min-Max normalized median price [0,1] used for ECR calculation. |
