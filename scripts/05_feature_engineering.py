...
def generate_candidates(
    ...
    console.print(f"  [dim]feat_max_date = {feat_max_date}, target_users = {len(target_users):,}[/dim]")
    ...
    pop_items  = (
        ...
        .group_by("article_id").agg(pl.col("quantity").sum().alias("cnt"))
        .sort("cnt", descending=True).head(50)["article_id"].to_list()
    )
    pop = (
        pl.DataFrame({"customer_id": target_users}, schema={"customer_id": pl.Int32})
        .join(pl.DataFrame({"article_id": pop_items}, schema={"article_id": pl.Int32}), how="cross")
    ...
    iu_sp = ui_sp.T.tocsr()
    del rows, cols, feat_6w; gc.collect()
    ...
    del sim_dict; gc.collect()

    cf_cands = (
        pl.DataFrame(cf_rows)
        if cf_rows
        else pl.DataFrame(schema={"customer_id": pl.Int32, "article_id": pl.Int32, "src": pl.Utf8})
    )
    ...
    cast_expr = [pl.col("customer_id").cast(pl.Int32), pl.col("article_id").cast(pl.Int32)]
    all_cands = pl.concat([
        repurch.select(["customer_id", "article_id", "src"]).with_columns(cast_expr),
    ...
    all_cands = (
        all_cands
        .with_columns(pl.col("article_id").cum_count().over("customer_id").alias("rn"))
...
def build_features(
    ...
    ui_1w = feat_df.filter(pl.col("t_dat") >= w1).group_by(["customer_id", "article_id"]).agg(
        (pl.col("quantity").cast(pl.Float32) * pl.col("time_weight")).sum().alias("ui_tw_1w")
    )
    ui_1m = feat_df.filter(pl.col("t_dat") >= m1).group_by(["customer_id", "article_id"]).agg(
        (pl.col("quantity").cast(pl.Float32) * pl.col("time_weight")).sum().alias("ui_tw_1m")
    )
    ui_1s = feat_df.filter(pl.col("t_dat") >= s1).group_by(["customer_id", "article_id"]).agg(
        (pl.col("quantity").cast(pl.Float32) * pl.col("time_weight")).sum().alias("ui_tw_1s")
    )
    ...
    del ui_full, ui_1w, ui_1m, ui_1s; gc.collect()
    ...
    buyer_age = (
        feat_df.select(["customer_id", "article_id"]).unique()
    ...
    del item_price, buyer_age; gc.collect()
    ...
    cands = cands.join(customers_df.select(["customer_id", "age"]), on="customer_id", how="left")
    del user_agg; gc.collect()

    # 2.4 Word2Vec similarity
    console.print("  [dim]Training Word2Vec...[/dim]")
    sequences = feat_df.sort("t_dat").group_by("customer_id").agg(pl.col("article_id").alias("seq"))
    sentences = [list(map(str, s)) for s in sequences["seq"].to_list()]
    w2v = Word2Vec(sentences, vector_size=64, window=5, min_count=3, sg=1, workers=4, seed=RS, epochs=5)
    ...
    del sentences, sequences; gc.collect()

    cands_pd  = cands.to_pandas()
    cos_sims  = np.zeros(len(cands_pd), dtype=np.float32)
    for i, (uid, aid) in enumerate(zip(cands_pd["customer_id"], cands_pd["article_id"])):
    ...
    del cos_sims, user_emb_dict, item_embs, w2v; gc.collect()

    # 2.5 Cumulative features (inactive users = no purchase in last 30 days)
    recent_users  = set(feat_df.filter(pl.col("t_dat") >= m1)["customer_id"].unique().to_list())
    inactive_mask = ~cands_pd["customer_id"].isin(recent_users)
    full_hist_pd  = feat_df.group_by(["customer_id", "article_id"]).agg([
    ...
    if inactive_mask.any():
        for idx in cands_pd.index[inactive_mask]:
            key = (int(cands_pd.at[idx, "customer_id"]), int(cands_pd.at[idx, "article_id"]))
            if key in full_hist_pd.index:
                cands_pd.at[idx, "cum_purchase_count"] = full_hist_pd.at[key, "cum_purchase_count"]
                cands_pd.at[idx, "cum_avg_price"]      = full_hist_pd.at[key, "cum_avg_price"]
    del full_hist_pd; gc.collect()
...
def reduce_mem(df: pd.DataFrame) -> pd.DataFrame:
    for c in df.columns:
        ...
        if col.dtype == "float64":
            ...
            if   mn >= np.iinfo(np.int8).min  and mx <= np.iinfo(np.int8).max:  df[c] = col.astype(np.int8)
            elif mn >= np.iinfo(np.int16).min and mx <= np.iinfo(np.int16).max: df[c] = col.astype(np.int16)
            else: df[c] = col.astype(np.int32)
    return df
...
def finalize(cands_pd: pd.DataFrame, is_train: bool) -> pd.DataFrame:
    ...
    if is_train:
        ...
        cands_pd = pd.concat([pos, neg], ignore_index=True).sample(frac=1, random_state=RS).reset_index(drop=True)
...
def main() -> None:
    ...
    console.print(f"  Train date range : {train_full['t_dat'].min()} → {train_max_date}")
    console.print(f"  Test date range  : {test_full['t_dat'].min()} → {test_full['t_dat'].max()}")
    ...
    label_pairs = (
        train_labels.select(["customer_id", "article_id"]).unique()
        .with_columns(pl.lit(1).alias("label").cast(pl.Int8))
    )
    train_cands = train_cands.join(label_pairs, on=["customer_id", "article_id"], how="left").with_columns(
    ...
    console.print(f"  Shape : {train_pd.shape}  |  Pos: {(train_pd['label']==1).sum():,}  |  Neg: {(train_pd['label']==0).sum():,}")
    console.print(f"  Time  : {time.time()-t0:.1f}s")
    del train_feat, train_labels, train_cands, train_pd; gc.collect()
    ...
    label_pairs_test = (
        test_full.select(["customer_id", "article_id"]).unique()
        .with_columns(pl.lit(1).alias("label").cast(pl.Int8))
    )
    test_cands = test_cands.join(label_pairs_test, on=["customer_id", "article_id"], how="left").with_columns(
        pl.col("label").fill_null(0).cast(pl.Int8)
    )
    ...
    console.print(f"  Shape : {test_pd.shape}  |  Users: {test_pd['customer_id'].nunique():,}")
    console.print(f"  Time  : {time.time()-t0:.1f}s")
